 ...
 public:
   ...
   Recorder(std::string filename) {
     ...
     if (boost::filesystem::create_directories(dir.parent_path())) {
       PRINT_INFO("Created folder path to output file.");
       PRINT_INFO("Path: %s", dir.parent_path().c_str());
     }
     // If it exists, then delete it
     if (boost::filesystem::exists(filename)) {
       PRINT_WARNING("Output file exists, deleting old file....");
     ...
     if (outfile.fail()) {
       ...
       PRINT_ERROR("Path: %s", filename.c_str());
